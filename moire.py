...
def svg_line(x0, y0, x1, y1, th=0, cls=None):
    ...
    return '<line x1="{}" y1="{}" x2="{}" y2="{}" {}/>'.format(x0, y0, x1, y1, s_cls)

def add_unit_cell(s, x0, y0, th=0, cls=None):
    """Add a unit cell from the lattice to the SVG output.

    The "unit cell" consists of the arrangement of lines like "Y"
...
def svg_preamble(s):
    ...
    s.append('<svg xmlns="http://www.w3.org/2000/svg"\n' + ' '*5 + 'xmlns:xlink="http://www.w3.org/1999/xlink" width="{}" height="{}" >'.format(width, height))
    s.append("""<defs>
