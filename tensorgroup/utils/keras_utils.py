...
class TimeHistory(tf.keras.callbacks.Callback):
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        if self.global_steps % self.log_steps == 0:
            ...
            logging.info("BenchmarkMetric: {'global step':%d, 'time_taken': %f,'examples_per_second': %f}",self.global_steps, elapsed_time, examples_per_second)
...
def get_profiler_callback(model_dir, profile_steps, enable_tensorboard,steps_per_epoch):
    ...
    if start_step < 0 or start_step > stop_step: raise ValueError(profile_steps_error_message)
    if enable_tensorboard: logging.warning(
...
class ProfilerCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        if epoch == self.start_epoch: self.should_start = True
        if epoch == self.stop_epoch:  self.should_stop = True
...
def set_session_config(enable_eager=False,
    ...
    if is_v2_0():set_config_v2(enable_xla=enable_xla)
    else:
        config = get_config_proto_v1(enable_xla=enable_xla)
        if enable_eager:tf.compat.v1.enable_eager_execution(config=config)
        else: 
